...
from .target_assigner.proposal_target_layer_da import ProposalTargetLayerDA

class PointRCNNHeadDA(RoIHeadTemplate):
    def __init__(self, input_channels, model_cfg, num_class=1):
        super().__init__(num_class=num_class, model_cfg=model_cfg)
        self.model_cfg = model_cfg
        use_bn = self.model_cfg.USE_BN
        self.SA_modules = nn.ModuleList()
        # self.SA_modules_img = nn.ModuleList()
        channel_in = input_channels

        # Overriding parent proposal_target_layer
        self.proposal_target_layer = ProposalTargetLayerDA(roi_sampler_cfg=self.model_cfg.TARGET_CONFIG)
        ...
        # channel_in = input_channels
        # for k in range(self.model_cfg.SA_CONFIG_IMG.NPOINTS.__len__()):
        #     mlps = [channel_in] + self.model_cfg.SA_CONFIG_IMG.MLPS[k]
        #
        #     npoint = self.model_cfg.SA_CONFIG_IMG.NPOINTS[k] if self.model_cfg.SA_CONFIG_IMG.NPOINTS[k] != -1 else None
        #     self.SA_modules_img.append(
        #         pointnet2_modules.PointnetSAModule(
        #             npoint=npoint,
        #             radius=self.model_cfg.SA_CONFIG_IMG.RADIUS[k],
        #             nsample=self.model_cfg.SA_CONFIG_IMG.NSAMPLE[k],
        #             mlp=mlps,
        #             use_xyz=True,
        #             bn=use_bn
        #         )
        ...
        # xyz_mlps = [self.num_prefix_channels] + self.model_cfg.XYZ_UP_LAYER_IMG
        # shared_mlps = []
        # for k in range(len(xyz_mlps) - 1):
        #     shared_mlps.append(nn.Conv2d(xyz_mlps[k], xyz_mlps[k + 1], kernel_size=1, bias=not use_bn))
        #     if use_bn:
        #         shared_mlps.append(nn.BatchNorm2d(xyz_mlps[k + 1]))
        #     shared_mlps.append(nn.ReLU())
        # self.xyz_up_layer_img = nn.Sequential(*shared_mlps)

        # c_out = self.model_cfg.XYZ_UP_LAYER_IMG[-1]
        # self.merge_down_layer_img = nn.Sequential(
        #     nn.Conv2d(c_out * 2, c_out, kernel_size=1, bias=not use_bn),
        #     *[nn.BatchNorm2d(c_out), nn.ReLU()] if use_bn else [nn.ReLU()]
        # )
    ...
    def create_img_features_old(self, batch_dict):
        batch_size = batch_dict['batch_size']
        batch_idx = batch_dict['point_coords'][:, 0]
        point_coords = batch_dict['point_coords'][:, 1:4]
        point_features = batch_dict['point_features']
        ...
        point_scores = batch_dict['point_cls_scores'].detach()
        point_depths = point_coords.norm(dim=1) / self.model_cfg.ROI_POINT_POOL.DEPTH_NORMALIZER - 0.5
        xyz_input = [point_scores.view(batch_size, -1, 1),
                     point_depths.view(batch_size, -1, 1),
                     point_coords.view(batch_size, -1, 3)]
        xyz_input = torch.cat(xyz_input, dim=2).transpose(1, 2).unsqueeze(dim=3)
        xyz_features = self.xyz_up_layer_img(xyz_input)  # (batch_size, num_features, num_points, 1)
        point_features = point_features.view(batch_size, -1, point_features.shape[-1]).transpose(1, 2).unsqueeze(dim=3)
        merged_features = torch.cat((xyz_features, point_features), dim=1)
        merged_features = self.merge_down_layer_img(merged_features)  # (batch_size, num_features, num_points, 1)

        batch_points = point_coords.view(batch_size, -1, 3)
        l_xyz, l_features = [batch_points.contiguous()], [merged_features.squeeze(dim=3).contiguous()]

        for i in range(len(self.SA_modules_img)):
            li_xyz, li_features = self.SA_modules_img[i](l_xyz[i], l_features[i])
            l_xyz.append(li_xyz)
            l_features.append(li_features)

        shared_features = l_features[-1]
        batch_dict['img_features'] = shared_features.squeeze(dim=2)
    ...
    def forward(self, batch_dict):
        ...
        if self.training:
            ...
            batch_dict['roi_domain_labels'] = targets_dict['roi_domain_labels']
        ...
        merged_features = torch.cat((xyz_features, point_features), dim=1)
        merged_features = self.merge_down_layer(merged_features)  # (total_rois, num_features, num_sampled_points, 1)
        ...
        batch_size = batch_dict['batch_size']
        batch_dict['roi_features'] = shared_features.squeeze(dim=2).view(batch_size, -1, shared_features.shape[1])

        rcnn_cls = self.cls_layers(shared_features).transpose(1, 2).contiguous().squeeze(dim=1)  # (B, 1 or 2)
        rcnn_reg = self.reg_layers(shared_features).transpose(1, 2).contiguous().squeeze(dim=1)  # (B, C)

        # self.create_img_features_old(batch_dict)
