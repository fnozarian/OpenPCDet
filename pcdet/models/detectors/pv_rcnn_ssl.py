...
class PVRCNN_SSL(Detector3DTemplate):
    ...
    def calc_statistics_new(self):
        ...
        axs[0][0].hist(variances.cpu()[:, 0], range=[0, .5])
        axs[0][0].set_xlabel('var of x')
        axs[0][0].set_ylabel('Counts')
        axs[0][1].hist(variances.cpu()[:, 1], range=[0, .5])
        axs[0][1].set_xlabel('var of y')
        axs[1][0].hist(variances.cpu()[:, 2], range=[0, .5])
        axs[1][0].set_xlabel('var of z')
        axs[1][1].hist(variances.cpu()[:, 3], range=[0, .5])
        axs[1][1].set_xlabel('var of dx')
        axs[2][0].hist(variances.cpu()[:, 4], range=[0, .5])
        axs[2][0].set_xlabel('var of dy')
        axs[2][1].hist(variances.cpu()[:, 5], range=[0, .5])
        axs[2][1].set_xlabel('var of dz')
        axs[3][0].hist(variances.cpu()[:, 6], range=[0, .5])
    ...
    def ensemble_post_processing(self, batch_dict_a, batch_dict_b, unlabeled_inds, ensemble_option=None):
        ...
        if ensemble_option == 'joint_nms':
            ...
            self.compare_pair_metric.update(batch_dict_a, batch_dict_b, ('batch_cls_preds', 'batch_box_preds'), unlabeled_inds)
