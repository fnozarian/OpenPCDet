...
class DataBaseSamplerPL(object):
    def __init__(self, root_path, sampler_cfg, class_names, logger=None):
        ...
        self.db_infos, self.sample_groups, self.sample_class_num= self.__get_infos__(db_info_path)

        # PL
        for db_pl_info_path in self.sampler_cfg.DB_PL_INFO_PATH:
            db_pl_info_path = self.root_path.resolve() / db_pl_info_path
        self.db_pl_infos, self.sample_pl_groups, self.sample_pl_class_num= self.__get_infos__(db_pl_info_path)
    ...
    def sample_with_fixed_number(self, class_name, sample_group, db_infos):
        """
        Args:
            class_name:
            sample_group:
        ...
        if pointer >= len(db_infos[class_name]):
            indices = np.random.permutation(len(db_infos[class_name]))
            pointer = 0

        sampled_dict = [db_infos[class_name][idx] for idx in indices[pointer: pointer + sample_num]]
        pointer += sample_num
    ...
    def __get_infos__(self, db_info_path):
        db_infos = {}
        sample_groups = {}
        sample_class_num = {}
        ...
        try:
            ...
            return db_infos, sample_groups, sample_class_num
    
        for func_name, val in self.sampler_cfg.PREPARE.items():
            db_infos = getattr(self, func_name)(self.db_infos, val)
        ...
        for x in self.sampler_cfg.SAMPLE_GROUPS:
            ...
            sample_groups[class_name] = {
                ...
                'pointer': len(db_infos[class_name]),
                'indices': np.arange(len(db_infos[class_name]))
                }

        return db_infos, sample_groups, sample_class_num

    def __call__(self, data_dict, no_db_sample=False, pl_sample=False):
        """
        Args:
            data_dict:
                gt_boxes: (N, 7 + C) [x, y, z, dx, dy, dz, heading, ...]
        ...
        if pl_sample:
            db_infos, sample_groups, sample_class_num = self.db_pl_infos, self.sample_pl_groups, self.sample_pl_class_num
        else:    
            db_infos, sample_groups, sample_class_num = self.db_infos, self.sample_groups, self.sample_class_num


        for class_name, sample_group in sample_groups.items():
            if self.limit_whole_scene:
                num_gt = np.sum(class_name == gt_names)
                sample_group['sample_num'] = str(int(sample_class_num[class_name]) - num_gt)
            if int(sample_group['sample_num']) > 0:
                sampled_dict = self.sample_with_fixed_number(class_name, sample_group, db_infos)
        ...
        if total_valid_sampled_dict.__len__() > 0 and (not no_db_sample or pl_sample):
