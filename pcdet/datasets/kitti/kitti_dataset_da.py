...
# TODO(farzad) write a dataset wrapper for adding source/target domain labels to dataset items
class KittiDatasetDA(DatasetTemplate):
    ...
    def include_kitti_data(self, mode):
        ...
        if self.logger is not None:
            self.logger.info('Total samples for %s dataset: %d' % (self.real_name, len(kitti_infos)))
    ...
    def __getitem__(self, index):
        ...
        if self.dataset_cfg.get('NORM_INTENSITY', False) and self.real_name == 'nuscenes':
            points[:, 3] = points[:, 3] / 255.0

        if self.dataset_cfg.get('ZERO_INTENSITY', False) and self.real_name == 'nuscenes':
            points[:, 3] = 0
...
def create_kitti_infos(dataset_cfg, class_names, data_path, save_path, workers=16):
    dataset = KittiDatasetDA(dataset_cfg=dataset_cfg, class_names=class_names, root_path=data_path, training=False)
...
if __name__ == '__main__':
    ...
    if sys.argv.__len__() > 1 and sys.argv[1] == 'create_kitti_infos':
        ...
        create_kitti_infos(
            ...
            data_path=ROOT_DIR / 'data' / 'nusc-in-kitti-format',
            save_path=ROOT_DIR / 'data' / 'nusc-in-kitti-format'
        )
