...
def train_ssl_one_epoch(model, optimizer, labeled_loader, unlabeled_loader, epoch_id,
                        ...
                        rank, tbar, total_it_each_epoch, labeled_loader_iter, unlabeled_loader_iter, tb_log=None,
                        leave_pbar=False, dist=False):
    if rank == 0:
    ...
    for cur_it in range(total_it_each_epoch):
        ...
        loss, tb_dict, disp_dict = model.forward(ld_teacher_batch_dict, ud_teacher_batch_dict, ld_student_batch_dict,
...
def train_ssl_model(model, optimizer, labeled_loader, unlabeled_loader, lr_scheduler, ssl_cfg, start_epoch,
                    total_epochs, start_iter, rank, tb_log, ckpt_save_dir, labeled_sampler, unlabeled_sampler,
        ...
        for cur_epoch in tbar:
            ...
            if trained_epoch % ckpt_save_interval == 0 and rank == 0:
                ...
                if dist:
                    ...
                    save_checkpoint(
                        checkpoint_state(model.student, optimizer, trained_epoch, accumulated_iter),
                        filename=student_ckpt_name,
                    )
                ...
                if dist:
                    ...
                    save_checkpoint(
                        checkpoint_state(model.teacher, optimizer, trained_epoch, accumulated_iter),
                        filename=teacher_ckpt_name,
                    )
